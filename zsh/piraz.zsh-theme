...
# git theming
ZSH_THEME_GIT_PROMPT_PREFIX="%{$fg_bold[green]%}(%{$fg_no_bold[red]%}%B"
...
local prompt_end="%{$fg[green]%}>>>%{$reset_color%}"
local git_p="${green_op}$(git_prompt_info)${green_cp}"
local path_p="${green_op}%{$fg_bold[white]%}pwd:%~${green_cp}"
local user_host="${green_op}%{$fg_bold[white]%}%n@%m${green_cp}"
local ret_status="${green_op}%{$fg_bold[white]%}ret:%?${green_cp}"
local hist_no="${green_op}%{$fg_bold[white]%}#%h${green_cp}"
local jobs_no="${green_op}jobs:%j${green_cp}"
local piraz_smirk="%(?,%{$fg_bold[white]%}P%)%{$reset_color%},%{$fg[red]%}P(%{$reset_color%})"
PROMPT="${user_host}${green_op}${piraz_smirk}${green_cp}${ret_status}
${path_p}$(git_prompt_info)
${hist_no}${prompt_end} "
# local cur_cmd="${green_op}%_${green_cp}"
# PROMPT2="${cur_cmd}> "
